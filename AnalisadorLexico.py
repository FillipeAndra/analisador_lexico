...
def token(input_string):
    ...
    while i < len(input_string):
        ...
        if char.isdigit():
            ...
            # Lógica para identificar operadores e pontuações
        elif char == '=':
            if input_string[i + 1] == '=':
            ...
            tokens.append((ABRE_CHAVES,char))
            ...
            tokens.append((ASPAS,char))
        # Atualizar o índice para verificar o próximo caractere
        i += 1
...
def analisador_lexico_arquivo(nome_arquivo):
    try:
        with open(nome_arquivo, 'r') as file:
            ...
            print(f"Linha | {'Token':^15} | {'Valor'}")
            print("-" * 40)
            for num_linha, linha in enumerate(linhas, start=1):
                tokens = token(linha)
                for token_type, token_value in tokens:
                    print(f"Linha {num_linha:3} | {token_type:^15} | '{token_value}'")
...
analisador_lexico_arquivo('codigo.txt')